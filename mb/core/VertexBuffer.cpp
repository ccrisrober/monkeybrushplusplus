...
namespace mb
{
  ...
  std::vector<float> VertexBuffer::getSubData(int size, int offset)
  {
    ...
    void VertexBuffer::attribDivisor(int position, int length, int divisor,
      const void* stride)
  {
    this->bind();
    glEnableVertexAttribArray(position);
    glVertexAttribPointer(position, length, GL_FLOAT, false,
      length * sizeof(float), (const void*)stride);
  ...
  {
    this->bind();
    glBufferData(this->_type, _data.size() * sizeof(unsigned int),
      _data.data(), usage);
  }
        ...
        unsigned int type, bool normalized, const void* stride)
  {
    ...
    glVertexAttribPointer(
      ...
      (const void*)stride // Offset from the beginning of a single
