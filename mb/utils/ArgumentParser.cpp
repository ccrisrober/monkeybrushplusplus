...
namespace MB
{
	...
	ArgumentParser& ArgumentParser::add_option_group(const OptionGroup& group) {
	...
	const Option& ArgumentParser::lookup_short_opt(const std::string& opt) const {
	...
	void ArgumentParser::handle_short_opt(const std::string& opt, const std::string& arg) {
	...
	const Option& ArgumentParser::lookup_long_opt(const std::string& opt) const {
	...
	void ArgumentParser::handle_long_opt(const std::string& optstr) {
	...
	Values& ArgumentParser::parse_args(const int argc, char const* const* const argv)
	{
		if (prog() == "")
			prog(basename(argv[0]));
		return parse_args(&argv[1], &argv[argc]);
	}
	Values& ArgumentParser::parse_args(const std::vector<std::string>& v)
	{
	...
	void ArgumentParser::process_opt(const Option& o, const std::string& opt, const std::string& value)
	{
		if (o.action() == "store")
		{
	...
	void ArgumentParser::set_usage(const std::string& u) {
		std::string lower = u;
	...
	std::string ArgumentParser::format_usage(const std::string& u) const {
		std::stringstream ss;
		ss << _("Usage") << ": " << u << std::endl;
		return ss.str();
	}
	std::string ArgumentParser::get_usage() const {
		if (usage() == SUPPRESS_USAGE)
			return std::string("");
		return format_usage(str_replace(usage(), "%prog", prog()));
	}
	void ArgumentParser::print_usage(std::ostream& out) const {
	...
	std::string ArgumentParser::get_version() const {
		return str_replace(_version, "%prog", prog());
	}
	void ArgumentParser::print_version(std::ostream& out) const {
	...
	void ArgumentParser::exit() const {
		std::exit(2);
	}
	void ArgumentParser::error(const std::string& msg) const {
	...
	std::string Option::format_help(unsigned int indent /* = 2 */) const {
		...
		ss << h;
		// if the option std::list is too long, start a new paragraph
