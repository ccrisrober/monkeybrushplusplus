...
#ifndef __MB_LAMBERT_MATERIAL__
#define __MB_LAMBERT_MATERIAL__
...
namespace MB
{
    ...
    {
    public:
        LambertMaterial(const Color3& diffuse, const Color3& ambient, const Color3& emissive)
        : Material()
        {
            _uniforms["projection"] = new Uniform(Matrix4);
            _uniforms["view"] = new Uniform(Matrix4);
            _uniforms["model"] = new Uniform(Matrix4);
			_uniforms["diffuse"] = new Uniform(Vector3, Vect3(diffuse));
			_uniforms["ambient"] = new Uniform(Vector3, Vect3(ambient));
			_uniforms["emissive"] = new Uniform(Vector3, Vect3(emissive));
